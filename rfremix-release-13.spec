...
Source:		rfremix-release-%{dist_version}.tar.lzma
Source1:	rfremix-install-media-dvd.repo
Source4:	rfremixconf.init

Source101:	kxkbrc
Source102:	10-set-layout-switcher-kbd-combination.sh

Obsoletes:	redhat-release
Provides:	redhat-release
Provides:	system-release = %{epoch}:%{version}-%{release}
Provides:	fedora-release = %{epoch}:%{version}-%{release}
Requires:	rfremix-config = %{epoch}:%{version}-%{release}
...
%description
Russian Fedora Remix release files such as yum configs and
various /etc/ files that define the release.


%package -n rfremix-config
Summary:	Apply Russian Fedora Remix configuration changes
Group:		System Environment/Base
Requires(post):	rfremix-release = %{epoch}:%{version}-%{release}
...
%description -n rfremix-config
This packages contains russianfedoraremixconf init script what applies some
changes to Russian Fedora Remix configuration
...
install -m 755 %{SOURCE4} $RPM_BUILD_ROOT/etc/rc.d/init.d/rfremixconf

# make skel
install -dD $RPM_BUILD_ROOT/etc/skel/.kde/share/config
install -dD $RPM_BUILD_ROOT/etc/X11/xinit/xinitrc.d

install -m644 %{SOURCE101} \
    $RPM_BUILD_ROOT/etc/skel/.kde/share/config

install -m755 %{SOURCE102} \
...
%files -n rfremix-config
%defattr(-,root,root,-)
%doc GPL
%{_sysconfdir}/rc.d/init.d/rfremixconf
%{_sysconfdir}/skel/.kde/share/config
%attr(0755, root, root) %{_sysconfdir}/X11/xinit/xinitrc.d/*


%changelog
* Thu Aug 19 2010 Arkady L. Shane <ashejn@yandex-team.ru> - 13-3
- bump epoch
- fix release version (to 13)

* Thu Aug 19 2010 Arkady L. Shane <ashejn@yandex-team.ru> - 13.1-1
- update to 13.1

* Fri Jun  4 2010 Arkady L. Shane <ashejn@yandex-team.ru> - 13-2
- fix media dvd path

* Thu May 13 2010 Arkady L. Shane <ashejn@yandex-team.ru> - 13-1
- final RFRemix 13

* Mon Mar 15 2010 Arkady L. Shane <ashejn@yandex-team.ru> - 13-0.6
- update to RFRemix 13
